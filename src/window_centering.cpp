...
namespace gr_ht::window_centering {
...
// UE4's render window class, carried by both shipped exes, Steam and GDK. The
// only other windows the process was seen to own are an invisible IME pair,
// which the visibility and owner tests in PickRenderWindow drop on their own - but the exes
// also carry a "SplashScreenClass", and core's FindGameWindow takes the first
// visible unowned window over 200x200 whatever its class, then centres once per
// process. Matching the class exactly is what keeps that one move on the render
// window rather than on whichever window the engine happens to put up first.
