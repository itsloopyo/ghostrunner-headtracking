...
void OpenLog() {
    ...
    // rotates the outgoing one to GhostrunnerHeadTracking.prev.log first. It
    // warns by itself when the rotation fails, so there is nothing to do here
    // but name the file.
    Log::Open(exeDir + L"\\GhostrunnerHeadTracking.log");
    Log::Line("=== Ghostrunner Head Tracking v" GR_HT_VERSION " (UE4.26) ===");
}
...
void Initialize(HMODULE) {
    ...
    if (!g_bootstrapThread) {
        wchar_t line[128];
        swprintf_s(line, L"GhostrunnerHeadTracking: CreateThread for the bootstrap failed "
