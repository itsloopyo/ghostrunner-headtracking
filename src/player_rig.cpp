...
// The PlayerCharacter fields, resolved against the class they were first seen
// on; a different pawn class re-resolves.
struct PlayerFields {
    std::uintptr_t Class = 0;
    bool IsPlayer = false;
    ue_reflect::FieldInfo Camera;      // FollowCameraRef
    ue_reflect::FieldInfo Dead;        // bIsPlayerDead
    ue_reflect::FieldInfo CurrentFov;  // CurrentFOV
    ue_reflect::FieldInfo DefaultFov;  // DefaultFOV
};
...
void RefreshPlayerFields(std::uintptr_t pawn) {
    ...
    g_player.Class = cls;
    g_player.IsPlayer =
        ue_reflect::FindPropertyInChain(cls, "FollowCameraRef", g_player.Camera) &&
        g_player.Camera.Size == sizeof(std::uintptr_t) &&
        ue_reflect::FindPropertyInChain(cls, "bIsPlayerDead", g_player.Dead) &&
        g_player.Dead.BoolMask &&
        ue_reflect::FindPropertyInChain(cls, "CurrentFOV", g_player.CurrentFov) &&
        g_player.CurrentFov.TypeName == "FloatProperty" &&
        ue_reflect::FindPropertyInChain(cls, "DefaultFOV", g_player.DefaultFov) &&
        g_player.DefaultFov.TypeName == "FloatProperty";
    Log::Line("player-rig: pawn class %s is %s", ue::ObjectName(cls).c_str(),
              g_player.IsPlayer ? "the player character" : "not a PlayerCharacter");
}
...
Snapshot Read(std::uintptr_t controller) {
    ...
    s.Pawn = pawn;
    s.Camera = ComponentTransform(ReadPointer(pawn, g_player.Camera));

    // An unreadable flag reads as the blocking answer, which is the direction
    // that leaves the game stock.
    bool dead = true;
    s.Dead = !ue_reflect::ReadBool(pawn, g_player.Dead, dead) || dead;

    float current = 0.0f, base = 0.0f;
    s.HaveFov = ue::SafeReadFloat(pawn + g_player.CurrentFov.Offset, current) &&
                ue::SafeReadFloat(pawn + g_player.DefaultFov.Offset, base) &&
                std::isfinite(current) && std::isfinite(base);
    if (s.HaveFov) {
        s.CurrentFov = current;
        s.DefaultFov = base;
    }
    return s;
}
