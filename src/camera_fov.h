...
enum class AspectModel {
    ...
    //
    // This is the branch Ghostrunner takes: it reports
    // ULocalPlayer::AspectRatioAxisConstraint as 0, and its
    // FMinimalViewInfo::AspectRatio stays 16:9 whatever the window is, which is
    // what makes the two models differ at all - where the reference and the
    // viewport agree the formulas below are the same expression.
    //
    // Measured in game, on the Steam build, by turning the camera through a
    // known angle and cross-correlating the frame before against the frame
    // after. At a 1280x480 window (2.667:1, FOV 85, reference 16:9) this model
    // predicts a 10 degree yaw moving the image 82.1px and an 8 degree pitch
    // moving it 65.4px; the measured shifts were 81px and 66px. HorizontalFixed
    // would have predicted 123.2px and 98.2px, and reading the scalar as UE's
    // own vertical half-angle 46.2px and 36.8px. At 1280x720 the same test read
    // 126px against this model's 123.2px, where every model that keeps the
    // reference aspect agrees.
    VerticalFixed,
};
