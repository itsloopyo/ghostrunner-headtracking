...
// back. Every other caller keeps the clean rotation. The sword, the shuriken
// and the grapple do not read the view at all: they resolve their aim from
// PlayerCharacter::FollowCameraRef's world transform, and the mod never writes
// to that component.
...
void LogHeartbeat(const FrameReport& r, std::uintptr_t retRva) {
    ...
    Log::Line("heartbeat ret=0x%08llx tracking=%s verdict=%s gate=%s viewOff=%.1fcm/%.1fdeg "
              ...
              "constraint=%d refAspect=%.3f tan=(%.4f,%.4f) lean=%s",
              static_cast<unsigned long long>(retRva),
              g_trackingEnabled.load() ? "ON" : "OFF", Reason(r.Verdict),
...
void LogNonFinitePose(bool rotation) {
...
void LogZoomTerms(const FrameReport& r, std::uint64_t tick) {
...
void ShapePose(Pose& pose, float entry, float zoomFactor) {
    pose.yaw *= entry;
    pose.pitch *= entry;
    pose.roll *= entry;
    pose.x *= entry;
...
void ApplyFrame(std::uintptr_t controller, std::uintptr_t retRva, ue4::FVector* outLocation,
    ...
    // The base is the pawn's own DefaultFOV, which is where the game writes its
    // field-of-view setting. CurrentFOV is what the slide, dash and rift curves
    // have driven it to, and is the same number the render caller is handed, so
    // the factor reads 1.0000 whenever none of them is running.
    report.BaseFov = rig.HaveFov ? rig.DefaultFov : 0.0f;
    ...
    if (session && session->Update(dt)) {
        ...
        if (report.HavePose && !Finite3(yaw, pitch, roll)) {
            report.HavePose = false;
            LogNonFinitePose(true);
        }
    }

    LogZoomTerms(report, tick);
    LogFovReadable(report);

    report.Verdict = DecideTracking(report.Gate, g_trackingEnabled.load(std::memory_order_relaxed),
                                    report.HavePose);

    // The aim's own ray, cast into the world this frame. It starts at
    // PlayerCharacter::FollowCameraRef and runs along that component's forward,
    // which is what the sword sweep, the shuriken and the grapple all resolve
    // their direction from - not the drawn view, which carries the camera
    // manager's shakes on top. Done in every gameplay frame, not only tracked
    // ones, so the log compares like with like.
    const FVector aimOrigin = rig.Camera.Valid ? rig.Camera.Position : cleanLocation;
    const FVector aimDir = rig.Camera.Valid
        ? rig.Camera.Forward
        : ue::QuatRotateVec(cleanQ, FVector{1.0, 0.0, 0.0});
    aim_trace::Result hit;
    if (report.Gate.InGameplay) {
        hit = aim_trace::Cast(rig.Pawn, aimOrigin, aimDir, kMaxTraceCm);
    ...
    bool havePosition = session->GetPositionOffset(offX, offY, offZ);
    if (havePosition && !Finite3(offX, offY, offZ)) {
        havePosition = false;
        LogNonFinitePose(false);
    }

    Pose pose{yaw, pitch, roll, offX, offY, offZ};
