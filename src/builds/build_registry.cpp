...
namespace gr_ht::builds
{
    ...
    namespace
    {
        ...
        // the one an unmatched exe is read against: Ghostrunner's TimeDateStamp
        // is a real build date, so above or below it says whether the game or
        // the mod is the one that is behind. Comparing a Steam exe with another
        // store's build would say nothing, which is why the reference is chosen
        // per store.
        constexpr std::array<const BuildProfile*, 1> kKnownProfiles = {
            &kSteamProfile_20220624,
        };
        ...
        // Profile names start with their store. Ghostrunner ships
        // Ghostrunner-Win64-Shipping.exe on Steam, Epic and GOG; a Game Pass
        // build would be Ghostrunner-WinGDK-Shipping.exe.
