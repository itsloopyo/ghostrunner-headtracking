...
std::uintptr_t Target(const std::string& t) {
    ...
    if (t == "world") return ue_call::WorldOf(g_controller);
    if (t == "gamestate") return ReadPtrProp(ue_call::WorldOf(g_controller), "GameState");
    if (t == "gamemode") return ReadPtrProp(ue_call::WorldOf(g_controller), "AuthorityGameMode");
...
// The shot log's spawn half: where each projectile queued by OnShotEvent started.
void LogSpawnedProjectiles() {
...
// Once a petrace window has run out, stop observing and write what it counted.
void FinishProcessEventTraceWhenDue() {
...
void Poll(std::uintptr_t controller) {
    ...
    LogSpawnedProjectiles();
    FinishProcessEventTraceWhenDue();
    const std::uint64_t now = GetTickCount64();
    if (now - g_lastPollMs < kFilePollMs) return;
    g_lastPollMs = now;

    for (const std::string& l : TakeCommandLines()) Run(l, controller);
}
