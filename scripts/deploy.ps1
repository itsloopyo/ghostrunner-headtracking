...
.PARAMETER GamePath
    Ghostrunner install root. Omit to deploy to every detected install.
#>
...
# Both the Steam exe (Ghostrunner\Binaries\Win64) and the Xbox / Game Pass exe
# (Ghostrunner\Binaries\WinGDK) import WINMM.dll statically, so the loader goes in as
# winmm.dll beside whichever exe the install has, exactly as install.cmd does it.
$null = Invoke-DevDeployASILoader `
    -GameId 'ghostrunner' `
    -GameDisplayName 'Ghostrunner' `
    -BuildOutputPath (Join-Path $root 'build/Release') `
    -ModDllName 'GhostrunnerHeadTracking.asi' `
