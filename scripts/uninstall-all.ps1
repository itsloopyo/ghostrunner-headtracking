...
<#
.SYNOPSIS
    Runs uninstall.cmd against every local install of Ghostrunner.
...
$paths = @(Find-AllGamePaths -GameId 'ghostrunner')
if ($paths.Count -eq 0) {
    Write-Host 'No Ghostrunner installation found; nothing to uninstall.' -ForegroundColor Yellow
    exit 0
}
