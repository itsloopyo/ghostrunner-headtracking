...
bool SessionGoesNonFinite(float FakeReceiver::*axis) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (session.GetRotation(yaw, pitch, roll) && !gr_ht::Finite3(yaw, pitch, roll))
            nonFinite = true;
        if (session.GetPositionOffset(x, y, z) && !gr_ht::Finite3(x, y, z)) nonFinite = true;
    }
    return nonFinite;
}
...
void TestTheGuard() {
    ...
    CHECK(gr_ht::Finite3(0.0f, 0.0f, 0.0f));
    CHECK(gr_ht::Finite3(179.9f, -89.9f, -179.9f));
    CHECK(gr_ht::Finite3(kHuge, -kHuge, kHuge));
    CHECK(!gr_ht::Finite3(inf, 0.0f, 0.0f));
    CHECK(!gr_ht::Finite3(0.0f, -inf, 0.0f));
    CHECK(!gr_ht::Finite3(0.0f, 0.0f, nan));
}
...
void TestAnOrdinaryStreamStaysFinite() {
    ...
    for (int i = 0; i < 240; ++i) {
        ...
        if (!session.GetRotation(yaw, pitch, roll) || !gr_ht::Finite3(yaw, pitch, roll))
            allFinite = false;
        if (session.GetPositionOffset(x, y, z) && !gr_ht::Finite3(x, y, z)) allFinite = false;
    }
