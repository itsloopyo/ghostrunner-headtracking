...
void TestAnIndexInsideTheTableResolvesToItsSlot() {
    ...
    CHECK(gr_ht::ue_call::ObjectItem(0) == table.Item(0));
    CHECK(gr_ht::ue_call::ObjectItem(3) == table.Item(3));
    CHECK_MSG(gr_ht::ue_call::ObjectItem(4) == table.Item(4),
              "the first index of the second chunk comes from the second chunk pointer");
    CHECK(gr_ht::ue_call::ObjectItem(kObjectCount - 1) == table.Item(kObjectCount - 1));
}

void TestAnIndexPastTheEndOfTheTableIsRefused() {
    FakeTable table;
    Install(table, Layout());
    CHECK_MSG(gr_ht::ue_call::ObjectItem(kObjectCount) == 0,
              "one past the last live object is outside the table");
    CHECK(gr_ht::ue_call::ObjectItem(kObjectCount + 1) == 0);
    CHECK_MSG(gr_ht::ue_call::ObjectItem(0x7FFFFFFF) == 0,
...
void TestANegativeIndexIsRefused() {
    FakeTable table;
    Install(table, Layout());
    CHECK(gr_ht::ue_call::ObjectItem(-1) == 0);
    CHECK(gr_ht::ue_call::ObjectItem(-0x7FFFFFFF) == 0);
}
...
void TestAZeroDivisorInTheLayoutIsRefusedRatherThanDividedBy() {
    ...
    CHECK_MSG(gr_ht::ue_call::ObjectItem(0) == 0, "a zero chunk size must not be divided by");
    Install(table, Layout(kChunkElems, 0));
    CHECK_MSG(gr_ht::ue_call::ObjectItem(0) == 0, "a zero item size is not a table layout");
}

void TestNoRuntimeMeansNoTable() {
    ue::SetRuntime(0, 0, Layout());
    CHECK(gr_ht::ue_call::ObjectItem(0) == 0);
}
...
void TestAWeakPointerResolvesOnlyWhileItsSlotHoldsTheSameObject() {
    ...
    CHECK(gr_ht::ue_call::ResolveWeak(2, 7) == object);
    CHECK_MSG(gr_ht::ue_call::ResolveWeak(2, 8) == 0,
              "a slot reused since the pointer was taken resolves to nothing");
    CHECK_MSG(gr_ht::ue_call::ResolveWeak(2, 0) == 0, "serial 0 is an empty weak pointer");
    CHECK_MSG(gr_ht::ue_call::ResolveWeak(kObjectCount, 7) == 0,
